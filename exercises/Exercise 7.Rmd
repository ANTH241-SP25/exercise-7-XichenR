...
Let's test whether there are age varies by gender in this NHANES sample. What is the null hypothesis? What is the alternative hypothesis?
Two groups of gender have significant difference.
```{r}
...
#3. F-test to see if the variance is in the continuous variable is the same for the different levels of the categorical variable:
...
```
Does the T-test support or refute the alternative hypoithesis?
...
```
p=0.000008085 significantly different, p</= 0.05, there is 95% of chance that this relationship is true. This slight different is statistically significant.
Question: Does the above T-test support the null hypothesis or the alternative hypothesis?
Alternative hyputhesis that gender has impact on people's BMI. These two gender groups have significantly difference. 
```{r}
...
#The null hypothesis is the BMI is not impact by whether people have health insurance. The alternative hypothesis is that the BMI is impact by whether people have health insurance.
...
It supports the alternative hypothesis that whether people have health insurances will have impact to people's BMI.
...
#3. Test to see if the variance of the continous vatriable are equal across different levvels of the categorical variable: var.test(Contvar~Catvar, data=, alternative="two.sided")
...
```
The results show that there is a statistically significant difference between two groups of people who have health insurance and people who do not have health insurance.
...
```{r}
#a.descriptive statistic of DR1TKCAL by gender.
NHANESdata%>%
  drop_na(DR1TKCAL)%>%
  group_by(Gender)%>%
...
```
The t-test supports the alternative hypothesis that gender has impact on the amount of caloric intake in one day. The two gender groups have a statistically significant difference.
...
ggplot(NHANESdata, aes(x=BMIcat, y=DR1TKCAL))+geom_boxplot(fill="orange")+labs(x="BMI category", y="calories consumed per day", title="Daily calories by BMIcat")
...
The above test supports the alternative hypothesis that there is a significant difference. There is a significant difference between the groups of underweight people and healthy people, the groups of obsese people and underweight people, and the groups of overweight people and underweight people. 
```{r}
...
The results support the alternative hypothesis that there is a statistically significant difference. The household education levels have impact on the ratio of family income to poverty. There is a significant difference between the groups of education level 1 (less than high school degree) and education level 2 (high school grad/GED or some college.AA degree), the groups of education level 1 and education level 3 (college graduation or above), and the groups of education level 2 and education level 3.
...
dunn.test(x=NHANESdata$DR1TPROT, g=NHANESdata$HHFoodSec, kw=TRUE)
```
The results support the alternative hypothesis that the household food security has impact on daily protein consumption. There is a statistically significant difference between the groups of household food security level 1 (full food security) and level 2 (marginal food security), the groups of level 1 and level 3 (low food security), and the groups of level 1 and level 4 (very low food security).
